<?php
	...
	abstract class Base {		
		...
		protected function dbQuery($q){
			...
			if(!empty($q)){
				...
				if(!$r){ //return empty result on fail and email webmaster
					...
					$r = mysqli_query($this->dbcs->dbc, "SELECT 1 FROM `dwgreen_cms`.`ads` WHERE 1=2");
				}
				return $r;
			}else{
				return false;
		...
		protected function dbColumns($table,$db="dwgreen_cms"){
		...
		protected function sendError($q,$msg){
			
			//email mysql error to webmaster
			mailout("webmaster@dwgreen.com","bad query in ".get_class($this)." class","Bad query on page http://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."<br><br>SQL:<br>$q<br><br>MYSQLI ERROR:<BR>$msg<br><br>".json_encode($_SERVER),"webmaster@dwgmail.com");
			return $msg;
		}
...
###
	...
	class DBConnectSingleton {
		...
		public function setCreds($db_username,$db_password,$db_name="dwgreen_cms",$db_host="localhost"){
			
			$this->db_username = $db_username;
			$this->db_password = $db_password;
			$this->db_name = $db_name ?: "dwgreen_cms";
